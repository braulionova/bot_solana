//! tpu_sender.rs – Direct transaction submission to validator TPU ports.
//!
//! Bypasses JSON-RPC sendTransaction entirely.
//! Sends raw transactions via QUIC directly to the current slot leader(s).
...
impl TpuSender {
    ...
    pub async fn new(rpc_url: &str, ws_url: &str) -> anyhow::Result<Self> {
        ...
        let client = TpuClient::new("sim-server", rpc_client, ws_url, config)
            .await
            .map_err(|e| anyhow::anyhow!("TpuClient init: {}", e))?;

        info!(rpc = %rpc_url, ws = %ws_url, fanout = 2, "TpuClient ready");
        Ok(Self {
            client: Arc::new(client),
        })
    }

    /// Send raw transaction bytes directly to the current leader via QUIC/UDP.
    /// Returns the transaction signature as a base58 string.
    pub async fn send_wire(&self, tx_bytes: Vec<u8>) -> anyhow::Result<String> {
        // Deserialize only to extract the signature for logging/return.
        let tx: VersionedTransaction =
            bincode::deserialize(&tx_bytes).context("deserialize tx bytes for sig extraction")?;
        let sig = tx
            .signatures
            .first()
            .map(|s| s.to_string())
        ...
        if ok {
            ...
            // try_send_wire_transaction gives us the actual transport error
