...
└── src/
    └── main.rs                     # Orquestador: spawn tasks para cada fuente
```

---

## 6. Dependencias Principales (Cargo.toml workspace)
...
async fn main() -> Result<()> {
    ...
    // 5. Spawn Strategy Engine (consume de strategy_rx)
...
## 8. Comparativa de Fuentes — Resumen
...
| **Best for** | Whale backrun, pre-exec | Failover de F1 , pool state | Resiliencia multi-proveedor |
...
### Fase 5: Strategy Engine Connection (Semanas 9-10)
...
- [ ] Documentar latencias reales y win rates por fuente

---

## 10. Configuración Completa de Ejemplo
...
## 11. Requisitos de Hardware — Resumen por Configuración
...
4. `crates/solanacdn-client` — requiere análisis del repo SolanaCDN primero
5. `crates/spy-node` — el más complejo, ya tiene diseño del Plan Maestro
...
### Systemd Services
```
helios-ingest.service    — Binario principal (todas las fuentes + bus + strategy)
